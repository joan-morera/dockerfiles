...
### Hardening Flags
The following flags are used to harden the binary against common exploits. None are "unnecessary" for a secure production environment, though they might have a negligible performance cost compared to an insecure build.

| Flag | Purpose | Why it is needed |
| :--- | :--- | :--- |
| `-fstack-protector-strong` | Stack Smashing Protection | adds guard variables to functions to detect stack buffer overflows. |
| `-D_FORTIFY_SOURCE=2` | Buffer Overflow Protection | Uses lighter-weight runtime checks for buffer overflows in common string/memory functions. |
| `-Wformat -Werror=format-security` | Format String Protection | Treats format string vulnerabilities (like `printf(user_input)`) as compilation errors. |
| `-Wl,-z,relro` | Read-only Relocations | Marks Global Offset Table (GOT) as read-only after loading to prevent function pointer hijacking. |
| `-Wl,-z,now` | Immediate Binding | Resolves all symbols at startup, allowing the entire GOT to be marked read-only (Full RELRO). |
